...
struct Move {
  int face;
  int dir;
}; // face: 0=L,1=R,2=U,3=D,4=F,5=B  dir: 1 ou -1
...
SolverResult solverResultIDAStar;
std::atomic<bool> solverRunningIDAStar{false};
// =================================================================
...
static Cor colorToCor(const Color &c) {
  ...
  if (r > 0.5f && g > 0.5f && b < 0.3f)
    return Cor::AMARELO;
  if (r < 0.3f && g > 0.5f && b < 0.3f)
    return Cor::VERDE;
  if (r < 0.3f && g < 0.3f && b > 0.5f)
    return Cor::AZUL;
  if (r > 0.5f && g < 0.3f && b < 0.3f)
    return Cor::VERMELHO;
  if (r > 0.5f && g > 0.2f && g < 0.7f && b < 0.2f)
    return Cor::LARANJA;
  return Cor::CINZA;
}
...
// consome o próximo movimento da fila (chamado quando nenhum movimento está
// ativo)
void dispatchNextShuffleMove() {
  bool isBusy = (L != 0 || R != 0 || U != 0 || D != 0 || F != 0 || B != 0);
  if (isBusy)
    return; // ainda animando, espera terminar
...
void drawShuffleGUI() {
  ...
  ImGui::SetNextWindowPos(ImVec2(20, displaySize.y - 20), ImGuiCond_Always,
                          ImVec2(0.0f, 1.0f));
  ImGui::SetNextWindowSize(ImVec2(300, 0), ImGuiCond_Always);
  ImGui::Begin("Embaralhar", nullptr,
  ...
  if (isShuffling) {
    ImGui::TextColored(
        ImVec4(1.0f, 0.8f, 0.0f, 1.0f), "Embaralhando... (%d restantes)",
        (int)shuffleQueue.size() + (L || R || U || D || F || B ? 1 : 0));
  } else {
...
void drawSolverGUI() {
    ...
    if (g_cubeRef) {
      for (int i = 0; i < 8; i++) {
        savedCubeState.pieces[i] = {
            (*g_cubeRef)[i].left,  (*g_cubeRef)[i].right,
            (*g_cubeRef)[i].top,   (*g_cubeRef)[i].bottom,
            (*g_cubeRef)[i].front, (*g_cubeRef)[i].back};
      }
  ...
  ImGui::TextColored(ImVec4(0.9f, 0.5f, 1.0f, 1.0f),
  ...
  if (solverRunningIDS.load()) {
    ...
    if (solverResultIDS.found) {
      ImGui::TextColored(ImVec4(0.85f, 0.5f, 1.0f, 1.0f),
                         "Solucao IDS encontrada!");
  ...
  if (ImGui::Button("Resolver por A*", ImVec2(-1, 30)) && canSolveIDAStar) {
    if (g_cubeRef && !solverRunningIDAStar.load()) {
      ...
      std::thread([estadoInicial]() {
        ...
        solverRunningIDAStar.store(false);
      }).detach();
    }
  }
  if (!canSolveIDAStar)
  ...
  if (solverRunningIDAStar.load()) {
    ImGui::TextColored(ImVec4(1.0f, 0.8f, 0.0f, 1.0f), "Buscando (IDS)...");
  } else if (solverResultIDAStar.ran) {
    if (solverResultIDAStar.found) {
      ImGui::TextColored(ImVec4(0.85f, 0.5f, 1.0f, 1.0f),
                         "Solucao IDS encontrada!");
      ImGui::Text("Nos visitados: %d", solverResultIDAStar.nodesVisited);
      ImGui::Text("Movimentos: %d", solverResultIDAStar.moveCount);
      ImGui::Spacing();
      ImGui::TextColored(ImVec4(1.0f, 0.85f, 0.3f, 1.0f), "Caminho IDS:");
      const std::string &p2 = solverResultIDAStar.path;
      const int lineLen2 = 35;
      for (size_t pos = 0; pos < p2.size(); pos += lineLen2) {
        ImGui::TextUnformatted(p2.substr(pos, lineLen2).c_str());
      }
...
void initCube(std::array<CubeSection, 8> &cube, Coord3d cubeOrigem) {
  ...
  cube[0] = {v[0], v[1],   v[2], v[3],  v[4], v[5], v[6],
             v[7], orange, gray, white, gray, gray, blue};
  v = calcCubeVertex(sectionsCenter[1], N);
  cube[1] = {v[0], v[1], v[2], v[3],  v[4], v[5], v[6],
             v[7], gray, red,  white, gray, gray, blue};
  v = calcCubeVertex(sectionsCenter[2], N);
  cube[2] = {v[0], v[1],   v[2], v[3],  v[4], v[5],  v[6],
             v[7], orange, gray, white, gray, green, gray};
  v = calcCubeVertex(sectionsCenter[3], N);
  cube[3] = {v[0], v[1], v[2], v[3],  v[4], v[5],  v[6],
             v[7], gray, red,  white, gray, green, gray};
  v = calcCubeVertex(sectionsCenter[4], N);
  cube[4] = {v[0], v[1],   v[2], v[3], v[4],   v[5], v[6],
             v[7], orange, gray, gray, yellow, gray, blue};
  v = calcCubeVertex(sectionsCenter[5], N);
  cube[5] = {v[0], v[1], v[2], v[3], v[4],   v[5], v[6],
             v[7], gray, red,  gray, yellow, gray, blue};
  v = calcCubeVertex(sectionsCenter[6], N);
  cube[6] = {v[0], v[1],   v[2], v[3], v[4],   v[5],  v[6],
             v[7], orange, gray, gray, yellow, green, gray};
  v = calcCubeVertex(sectionsCenter[7], N);
  cube[7] = {v[0], v[1], v[2], v[3], v[4],   v[5],  v[6],
             v[7], gray, red,  gray, yellow, green, gray};
}
...
int main() {
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    processInput(window);
    if (!isShuffling && !isPlayingSolution)
